...
ErrorDto error=new ErrorDto(r.getMessage(),new Date(),request.getDescription(false));
  ...
  /*  @ExceptionHandler(ArithmeticException.class)
    public ResponseEntity<ErrorDto> mathExc(ArithmeticException a, WebRequest request){
        ErrorDto error=new ErrorDto("not possible",new Date(),request.getDescription(false));
        return new ResponseEntity<>(error, HttpStatus.INTERNAL_SERVER_ERROR);
    }*/
